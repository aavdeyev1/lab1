...
int main( int argc, char* argv[] )
{
   ...
   size_t bytes = (bignum)n * sizeof(int);
    // Host vectors
    int *h_a;
    int *h_results;
   ...
   //  bignum len_a = N;
    ...
    // Allocate for device
    cudaMalloc(&d_a, (N + 1)* sizeof(bignum));
    cudaMalloc(&d_results, (N + 1) * sizeof(char));

    // Copy host vector to device
    cudaMemcpy( d_a, h_a, N*sizeof(bignum), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy( h_results, d_results, (N)*sizeof(bignum), cudaMemcpyDeviceToHost );
    printf("\nGPU ARRAY 2.0\n");
